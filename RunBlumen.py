...
if __name__ == '__main__':
    ...
    def get_model(size):
        ...
        model.add(tf.keras.layers.InputLayer(shape=(size,), name='input'))
        for i in range(layer):
            calculated_size = (int(i / 2)+ 1 ) * neuron_size
            model.add(tf.keras.layers.Dense(calculated_size, activation='relu', name='hidden'+str(i)))
            model.add(tf.keras.layers.Dropout(0.2))
        model.add(tf.keras.layers.Dense(1, activation='linear', name='output'))
        return model

    lr_reducer = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_r2_score', factor=0.2, patience=150, cooldown=150,
                                                      min_lr=0.00001)
